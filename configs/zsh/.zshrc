...
if [ "$color_prompt" = yes ]; then
    ...
    # My config prompt
    if [ $UID -ne 0 ]; then
        PROMPT=$'%F{%(#.blue.green)}${debian_chroot:+($debian_chroot)}%B%F{76}%n%(#.@.@)%m%b%F{reset}:%B%F{33}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}%B%(#.%F{red}#.%F{reset}$)%b%F{reset} '
    else
        PROMPT=$'%F{%(#.blue.green)}${debian_chroot:+($debian_chroot)}%B%F{160}%n%(#.@.@)%m%b%F{reset}:%B%F{33}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}%B%(#.%F{red}#.%F{reset}$)%b%F{reset} '
    fi
	...
	ZSH_HIGHLIGHT_STYLES[arg0]=fg=green # Color number "78" for green for "xtrem-256-color"
