# Sublime Merge 2

## Installation `Sublime Merge`
* Site : [Sublime Merge](https://www.sublimemerge.com/)

Use bash script to install sublime merge 2 in `../sublime-text`
```console
$ bash subl-instal-apt.sh
```

## Instructions to active the License

Sublime Merge 2 build 2050-1 Linux
```bash
#! /usr/bin/bash
#
# Original Link : https://gist.github.com/rufoa/78e45d70f560f53678853c92dae2598a
#
# Run these commands one line at a time, not as a script

sudo -i
cd /opt/sublime_merge

sha256sum sublime_merge
# should be 5679bb00194d7149226ef83dd6d923064b1a12e3771adc02f8ec3973eef8d4d0 (rpm)
#        or f41fc4f15635ae840bdee52c2958f4e3e62fcf2203b0fde1607a06961539634f (other linux)

cp sublime_merge sublime_merge.bak

# swap public key
printf '' | dd of=sublime_merge bs=1 seek=337840 count=160 conv=notrunc

# swap xor key
printf '\x00' | dd of=sublime_merge bs=1 seek=3830918 count=1 conv=notrunc

# fix sha-2 check
printf '\x0e' | dd of=sublime_merge bs=1 seek=3831617 count=1 conv=notrunc

# disable online licence check
printf '\xc3' | dd of=sublime_merge bs=1 seek=3835940 count=1 conv=notrunc

# disable phone home
printf '\xc3' | dd of=sublime_merge bs=1 seek=3830170 count=1 conv=notrunc

sha256sum sublime_merge
# should be 7f2e444c4c0571569433732691704e1df87eef6a697be2bcd82c8b53a38cd06e (rpm)
#        or a5fe3fabf4af71e777482af10409cf646b4c06a18249cb30b64065848b21a4aa (other linux)
```

Use the Key after script above.
```text
----- BEGIN LICENSE -----
TEAM RUFIO
Unlimited User License
E52D-666666
487EE6F0309908F702DDD52AFCD99A6A
6EE14CF8A2D42271B4FC0991BBF93ADC
FAA9075C436B3796669194A2F36CAAEF
B251155329EC2E434FD28B4A21BE68CC
955D306EE9ED843C5E98B1577D02DEAA
1F4E872AE6495CD5E3B1DA55D5ACD2B2
2EA4110FB800F21AA3EC2E3902589BCF
7281A19C2DFF0CEE4AEA5DDD1E6DF893
----- END LICENSE -----
```
